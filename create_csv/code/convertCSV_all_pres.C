...
void convertCSV(std::string inputfile, std::string outputfile)
{
    ...
    in.open(inputfile);
    Int_t fSec, fNanoSec;
    Double_t TOF_average_current, TOF_rms_current, luminosity, T0_luminosity, current, HV, mean_current, std_current, pressure;
    ...
    while (in >> fSec >> fNanoSec >> TOF_average_current >> TOF_rms_current >> luminosity >> T0_luminosity >> current >> HV >> pressure >> mean_current >> std_current)
    {
        tree->Fill();
    }
    in.close();
